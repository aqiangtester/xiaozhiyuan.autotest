...
//	private static String url = Constants.AWS_TEST_HOST + Constants.LOGIN_LOCATION;
	 ...
	 * @description 测试登录成功
	 */
	@Test(dataProvider = "testLoginSuccess", dataProviderClass = LoginDataProvider.class, groups = { "checkintest" } ,enabled=true)
	public void testLoginSuccess(Map<String, String> data) {
		...
		String requestParams = RequestParams.requestParams(data.get("account"), data.get("password"),
				ParamsParseInt.paramsParseInt(data.get("dataSource")), data.get("uniqueCode"));
	...
	public void testLoginFail(Map<String, String> data) {
		...
		String requestParams = RequestParams.requestParams(data.get("account"), data.get("password"),
				ParamsParseInt.paramsParseInt(data.get("dataSource")), data.get("uniqueCode"));
		logger.info(data.get("测试用例") + " : " + requestParams);
	...
	public void pressureTest(Map<String, String> data) {
		...
		ReadExcel.prmap(data);
		String requestParams = RequestParams.requestParams(data.get("account"), data.get("password"), ParamsParseInt.paramsParseInt(data.get("dataSource")));
